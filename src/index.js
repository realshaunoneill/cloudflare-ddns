...
const start = async () => {
  try {
    ...
    if (!isCloudflareTokenValid) {
      console.log(
        'Cloudflare API key is invalid. Exiting. Sent API key: ',
        CLOUDFLARE_API_KEY,
      );
      job.stop();
    ...
    const publicIpAddress = IP_OVERRIDE
      ? IP_OVERRIDE
      : await getPublicIpAddress();

    if (IP_OVERRIDE) {
      console.log(
        `IP Override: ${IP_OVERRIDE}, not checking public IP address.`,
      );
    } else {
      if (!publicIpAddress) {
    ...
    if (!zone) {
      sendWebhookRequest({
        status: 'error',
        message: `Unable to find zone for ${PARSED_TLD}. Please ensure that the domain is registered with Cloudflare.`,
      });
      console.log(
        `Unable to find zone for ${PARSED_TLD}. Please ensure that the domain is registered with Cloudflare.`,
      );
    ...
    if (!zoneRecords) {
      sendWebhookRequest({
        status: 'error',
        message: `Unable to get DNS records for ${PARSED_TLD}.`,
      });
      console.log(`Unable to get DNS records for ${PARSED_TLD}.`);
    }

    // Check if the DNS_URL is already in the zone
    const dnsRecord = zoneRecords.result.find(
      (record) => record.name === DNS_URL,
    );

    if (dnsRecord) {
      console.log(
        `DNS record already exists for ${DNS_URL}, checking if it needs to be updated...`,
      );

      // Check if the DNS record needs to be updated
      if (
        dnsRecord.content === publicIpAddress &&
        dnsRecord.proxied === PROXIED
      ) {
        console.log(
          `DNS record for ${DNS_URL} is already up to date. No action required.`,
        );
        sendWebhookRequest({
          status: 'success',
          message: `DNS record for ${DNS_URL} is already up to date. No action required.`,
        });
      } else {
        if (dnsRecord.content !== publicIpAddress) {
          console.log(
            `DNS record for ${DNS_URL} needs to be updated. IP Address: ${dnsRecord.content} -> ${publicIpAddress}`,
          );
        }
        if (dnsRecord.proxied !== PROXIED) {
          console.log(
            `DNS record for ${DNS_URL} needs to be updated. Proxied: ${dnsRecord.proxied} -> ${PROXIED}`,
          );
        }
        sendWebhookRequest({
          status: 'progress',
